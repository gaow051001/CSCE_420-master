...
for i in range(1,4):
    for j in range(len(color)):
        stepMessage = str(stepCounter) + ". "
        stepMessage += "-C%d%s v -C%d%s\n" % (i if i <= 3 else i % 3, color[j], i+1 if i+1 <= 3 else (i+1) % 3, color[j])
        f.write(stepMessage)
        stepCounter += 1

        stepMessage = str(stepCounter) + ". "
        stepMessage += "-C%d%s v -C%d%s\n" % (i if i <= 3 else i % 3, color[j], i+2 if i+2 <= 3 else (i+2) % 3, color[j])
        f.write(stepMessage)
        stepCounter += 1
...
for j in range(len(color)):
    ...
    for k in range(boxNum):
        #if k == 0: 
        stepMessage = str(stepCounter) + ". "
        stepMessage += "-L" + str(k+1) + color[j] + " " + orSymbol + " "
         #else:
...
for i in range(1,4):
    for color in ['W', 'Y']:
        stepMessage = str(stepCounter) + ". "
        stepMessage += "-O%d%s v C%d%s v C%dB\n" % (i, color, i, color, i)
        f.write(stepMessage)
        stepCounter += 1
